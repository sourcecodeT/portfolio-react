...
function Skills() {
  ...
  useEffect(() => {
    ...
    tl.staggerFrom(".skills_it", 1, { opacity: 0 }, 0.05, "+=.2")
      .staggerTo(".skills_it", 0.1, { color: "#59bd8e" }, 0.05, "-=1.3")
      .staggerTo(
        ".skills_it",
        0.3,
      ...
      .staggerTo(".skills_it", 0.5, { color: "#ffffff" }, 0.03, "-=1")
      .staggerTo(
        ".skills_it",
        0.5,
    ...
    ttl
      .fromTo(".skills_p", 1, { opacity: 0 }, { opacity: 1 }, "+=.2")
  ...
  function enterLink(type) {
    TweenMax.to(`.skills_link_${type}`, 0.4, { color: "white" });
  }

  function leaveLink(type) {
    TweenMax.to(`.skills_link_${type}`, 0.4, { color: "#59bd8e" });
  }

  function enterIT(type, index) {
    const tl = new TimelineMax();
    tl.to(`.skills_${type}_${index}`, 0.1, { color: "#59bd8e" });
    tl.to(
      `.skills_${type}_${index}`,
      0.3,
  ...
  function leaveIT(type, index) {
    const tl = new TimelineMax();
    tl.to(`.skills_${type}_${index}`, 0.5, { color: "#ffffff" });
    tl.to(
      `.skills_${type}_${index}`,
      0.5,
      { fontSize: "48px", ease: Power1.easeOut },
    ...
    return (
      <main className="Skills">
        <section className="skills_intro_section">
          ...
          <aside className="skills_h1">{renderTextIntro("My Skills")}</aside>
          <span className="skills_h1_span skills_span">{"<h1/>"}</span>
          <span className="skills_p_span skills_span">{"<p>"}</span>
          <p className="skills_p">
            I mainly work with front end technology (HTML5, CSS3, ES6
            JavaScript) to build scalable, responsive, single page web
            applications. I use ReactJS for most of my projects.
          </p>
          <span className="skills_br_span skills_span">{"</p> <p>"}</span>
          <p className="skills_p">
            For the back end, I usually build Express, GraphQL apps with NodeJS.
            I handle database management with MongoBD. I host all of my websites
            with NowJS as well.
          </p>
          <span className="skills_br_span skills_span">{"</p> <p>"}</span>
          <p className="skills_p">
            If you would like to know more, you can always
            <Link
              to="/contact"
        ...
        <section className="skills_wrap">
          <aside className="skills_cube">
            <div className="skills_cube_el skills_cube_front">
              {previousText}
            </div>
            <div className="skills_cube_el skills_cube_top">{currentText}</div>
            <div className="skills_cube_el skills_cube_bottom">
              {currentText}
            </div>
            <div className="skills_cube_el skills_cube_left">{currentText}</div>
            <div className="skills_cube_el skills_cube_right">
              {currentText}
            </div>
          </aside>
        </section>
